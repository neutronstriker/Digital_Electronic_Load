...
//#include "Wire.h" //All wire functions have been replaced by my i2c functions.
#include "I2C.H"
#define OLED_I2C_CLOCK I2C_BUS_CLK
...
*/
// initializer for I2C - by default there was reset pin defined which i removed
...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  _vccstate = vccstate;
  _i2caddr = i2caddr;
/* THis code is not necessary as it is related to SPI 
  // set pin directions
  if (sid != -1){
    pinMode(dc, OUTPUT);
    pinMode(cs, OUTPUT);
 ...
 */
/* I2C Reset Pin Related code, which is not there
	 if (reset) {
...
void Adafruit_SSD1306::ssd1306_command(uint8_t c) { 
 /* Again SPI related Code
  ...
  { */
	...
	/*
	i2c_start();
	i2c_tx_sla_w(_i2caddr);
    i2c_transmit(control);
	i2c_transmit(c);
    i2c_stop();
	*/
